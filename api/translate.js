...
`;
// ============================================================
// APPEND PATCH 01 — Enhanced Korean -> Thai hard map
// Paste this block at the VERY END of api/translate.js
// Do not delete old code. This function overrides the old one.
// ============================================================
...
function hardKoreanToThai(raw, compact, partnerGender) {
  ...
  const hasQ = /[?？]$/.test(r) || /(나요|습니까|세요|어요|예요|이에요|까요|ㄹ까요|을까요|있어요|없어요|돼요|되나요)$/.test(c);

  const q = (text) => `${text}${question}`;
  const p = (text) => `${text}${polite}`;

  // ============================================================
  // 0) Exact short common map
  // ============================================================
  ...
  const exactMap = {
    ...
    '미안해요': p('ขอโทษ'),
    '죄송합니다': p('ขอโทษ'),
    '괜찮아요': p('ไม่เป็นไร / โอเค'),
    '괜찮습니다': p('ไม่เป็นไร / โอเค'),
    '네': p('ใช่ / ค่ะ'),
    '예': p('ใช่ / ค่ะ'),
    '아니요': p('ไม่ใช่'),
    '아니에요': p('ไม่ใช่'),
    ...
    '맞습니다': p('ใช่ / ถูกต้อง'),
    '몰라요': p('ไม่รู้'),
    '모르겠어요': p('ไม่แน่ใจ / ไม่รู้'),
    '알겠어요': p('เข้าใจแล้ว'),
    '알겠습니다': p('เข้าใจแล้ว'),
    '잠깐만요': p('รอสักครู่'),
    '기다리세요': p('รอก่อน'),
    '천천히하세요': p('ค่อย ๆ ทำ'),
    '천천히말해주세요': p('ช่วยพูดช้า ๆ หน่อย'),
    '다시말해주세요': p('ช่วยพูดอีกครั้งหน่อย'),
    '조심하세요': p('ระวังนะ'),
    '빨리하세요': p('รีบทำหน่อย'),
    '하지마세요': p('อย่าทำ'),
    '하지마': 'อย่าทำ',
    '안돼요': p('ไม่ได้'),
    '돼요': p('ได้'),
    '됐어요': p('ได้แล้ว / พอแล้ว'),
    '필요없어요': p('ไม่จำเป็น / ไม่ต้องการ'),
    '상관없어요': p('ไม่เกี่ยว / ไม่เป็นไร'),
    '문제없어요': p('ไม่มีปัญหา'),

    // people / places
    '사장님': 'เถ้าแก่ / นายจ้าง / คุณ',
    '반장님': 'หัวหน้างาน',
    '팀장님': 'หัวหน้าทีม',
    '부장님': 'ผู้จัดการแผนก',
    '과장님': 'หัวหน้าแผนก / ผู้จัดการ',
    '선생님': 'คุณครู / คุณหมอ / อาจารย์',
    '회사': 'บริษัท',
    '공장': 'โรงงาน',
    '기숙사': 'หอพัก',
    '집': 'บ้าน',
    '학교': 'โรงเรียน / สถานศึกษา',
    '병원': 'โรงพยาบาล',
    '약국': 'ร้านขายยา',
    '은행': 'ธนาคาร',
    '출입국': 'สำนักงานตรวจคนเข้าเมือง',
    '노동부': 'กระทรวงแรงงาน',
    '대사관': 'สถานทูต',
    '경찰서': 'สถานีตำรวจ',
    '주민센터': 'ศูนย์บริการชุมชน / อำเภอเกาหลี',
    ...
    '근로계약서': 'สัญญาจ้างงาน',
    '급여명세서': 'สลิปเงินเดือน',
    '재직증명서': 'ใบรับรองการทำงาน',
    '진단서': 'ใบรับรองแพทย์',
    '처방전': 'ใบสั่งยา',
    '통장': 'สมุดบัญชี',
    '체크카드': 'บัตรเช็กการ์ด / บัตรเดบิต',
    '신용카드': 'บัตรเครดิต',

    // work nouns
    '월급': 'เงินเดือน',
    '급여': 'ค่าจ้าง / เงินเดือน',
    '퇴직금': 'เงินแทจิก / เงินชดเชยออกงาน',
    '국민연금': 'เงินกุกมิน / 국민연금',
    '건강보험': 'ประกันสุขภาพ',
    '고용보험': 'ประกันการจ้างงาน',
    '산재보험': 'ประกันอุบัติเหตุจากงาน',
    '세금': 'ภาษี',
    '환급': 'การขอคืนเงิน / เงินคืน',
    '야근': 'โอที / ทำงานล่วงเวลา',
    '잔업': 'งานล่วงเวลา',
    '특근': 'ทำงานพิเศษวันหยุด',
    '휴무': 'วันหยุด',
    '연차': 'วันลาพักร้อน',
    '병가': 'ลาป่วย',
    '지각': 'มาสาย',
    ...
    '취소': 'ยกเลิก',
    '판매자': 'ผู้ขาย / ร้านค้า',
    '고객센터': 'ศูนย์บริการลูกค้า',
    '운송장번호': 'เลขพัสดุ / เลขแทร็ก',
    '송장번호': 'เลขพัสดุ / เลขแทร็ก',
    ...
    '휴대폰': 'มือถือ',
    '인증번호': 'รหัสยืนยัน',
    '통신사': 'เครือข่ายมือถือ / บริษัทมือถือ',
    '미납': 'ยอดค้างชำระ',
    '자동이체': 'หักเงินอัตโนมัติจากบัญชี',

    // car
    '중고차': 'รถมือสอง',
    '자동차': 'รถยนต์',
    '명의이전': 'โอนชื่อ / โอนกรรมสิทธิ์',
    '주행거리': 'เลขไมล์ / ระยะทางวิ่ง',
    '보험': 'ประกัน',
    '자동차보험': 'ประกันรถยนต์',
    '사고차': 'รถเคยชน',
    '무사고': 'ไม่มีอุบัติเหตุ',
    '침수차': 'รถน้ำท่วม',
    '성능점검기록부': 'ใบตรวจสภาพรถ',
    '수리비': 'ค่าซ่อม',

    // food
    '물주세요': p('ขอน้ำหน่อย'),
    '계산해주세요': p('คิดเงินด้วย'),
    '포장해주세요': p('ห่อกลับด้วย'),
    '맛있어요': hasQ ? q('อร่อยไหม') : p('อร่อย'),
    '배고파요': p('หิว'),
    '밥먹었어요': hasQ ? q('กินข้าวแล้วหรือยัง') : p('กินข้าวแล้ว')
  };

  if (exactMap[c]) return exactMap[c];

  // ============================================================
  // 1) Time / schedule / coming / returning
  // ============================================================

  if (/몇시.*들어와/.test(c)) return q('จะเข้ามากี่โมง');
  if (/몇시.*돌아와/.test(c)) return q('จะกลับมากี่โมง');
  if (/몇시.*오세|몇시.*와요|몇시.*와/.test(c)) return q('จะมากี่โมง');
  if (/몇시.*가요|몇시.*가세/.test(c)) return q('ไปกี่โมง');
  if (/몇시.*출발/.test(c)) return q('ออกเดินทางกี่โมง');
  if (/몇시.*도착/.test(c)) return q('ถึงกี่โมง');
  if (/몇시.*끝나|몇시.*끝나요/.test(c)) return q('เสร็จกี่โมง');
  if (/몇시.*퇴근/.test(c)) return q('เลิกงานกี่โมง');
  if (/몇시.*출근/.test(c)) return q('เข้างานกี่โมง');
  if (/몇시까지.*와/.test(c)) return q('ต้องมาก่อนกี่โมง');
  if (/몇시까지.*끝/.test(c)) return q('ต้องเสร็จก่อนกี่โมง');
  if (/몇시까지.*해야/.test(c)) return q('ต้องทำถึงกี่โมง / ต้องทำให้เสร็จก่อนกี่โมง');

  if (/언제.*와/.test(c)) return q('จะมาเมื่อไหร่');
  if (/언제.*오세/.test(c)) return q('จะมาเมื่อไหร่');
  if (/언제.*들어와/.test(c)) return q('จะเข้ามาเมื่อไหร่');
  if (/언제.*돌아와/.test(c)) return q('จะกลับมาเมื่อไหร่');
  if (/언제.*출발/.test(c)) return q('จะออกเดินทางเมื่อไหร่');
  if (/언제.*도착/.test(c)) return q('จะถึงเมื่อไหร่');
  if (/언제.*끝나/.test(c)) return q('จะเสร็จเมื่อไหร่');
  if (/언제.*퇴근/.test(c)) return q('จะเลิกงานเมื่อไหร่');
  if (/언제.*출근/.test(c)) return q('จะเข้างานเมื่อไหร่');

  if (/오늘.*몇시.*퇴근/.test(c)) return q('วันนี้เลิกงานกี่โมง');
  if (/내일.*몇시.*출근/.test(c)) return q('พรุ่งนี้เข้างานกี่โมง');
  if (/오늘.*일찍.*끝/.test(c)) return p('วันนี้เลิกเร็ว / วันนี้เสร็จเร็ว');
  if (/오늘.*늦게.*끝/.test(c)) return p('วันนี้เสร็จช้า / วันนี้เลิกช้า');
  if (/시간.*얼마나.*걸/.test(c)) return q('ใช้เวลานานเท่าไหร่');
  if (/얼마나.*걸려/.test(c)) return q('ใช้เวลานานเท่าไหร่');
  if (/지금.*몇시/.test(c)) return q('ตอนนี้กี่โมงแล้ว');

  // ============================================================
  // 2) Place / direction / transportation
  // ============================================================

  if (/지금.*어디/.test(c)) return q('ตอนนี้อยู่ที่ไหน');
  if (/어디.*있/.test(c)) return q('อยู่ที่ไหน');
  if (/어디.*가/.test(c)) return q('ไปไหน');
  if (/어디로.*가/.test(c)) return q('ไปทางไหน');
  if (/여기.*어디/.test(c)) return q('ที่นี่ที่ไหน');
  if (/화장실.*어디/.test(c)) return q('ห้องน้ำอยู่ที่ไหน');
  if (/지하철역.*어디/.test(c)) return q('สถานีรถไฟใต้ดินอยู่ที่ไหน');
  if (/버스정류장.*어디/.test(c)) return q('ป้ายรถเมล์อยู่ที่ไหน');
  if (/택시.*불러/.test(c)) return p('เรียกแท็กซี่ให้หน่อย');
  if (/여기서.*내려/.test(c)) return p('ลงตรงนี้');
  if (/여기서.*세워/.test(c)) return p('จอดตรงนี้');
  if (/왼쪽/.test(c)) return 'ซ้าย';
  if (/오른쪽/.test(c)) return 'ขวา';
  if (/직진/.test(c)) return 'ตรงไป';
  if (/유턴/.test(c)) return 'กลับรถ';
  if (/길.*잃/.test(c)) return p('หลงทาง');
  if (/멀어요/.test(c)) return p('ไกล');
  if (/가까워요/.test(c)) return p('ใกล้');

  // ============================================================
  // 3) Work / factory / labor
  // ============================================================

  if (/출장/.test(c)) {
    if (/오늘/.test(c)) return p('วันนี้ไปทำงานนอกสถานที่');
    if (/내일/.test(c)) return p('พรุ่งนี้ไปทำงานนอกสถานที่');
    if (/어제/.test(c)) return p('เมื่อวานไปทำงานนอกสถานที่');
    if (/갈거|갈게|가야|갑니다|가요|간다/.test(c)) return p('ไปทำงานนอกสถานที่');
    if (/갔다|다녀왔|왔어요/.test(c)) return p('ไปทำงานนอกสถานที่มาแล้ว');
    if (/중/.test(c)) return p('กำลังทำงานนอกสถานที่');
    return 'ไปทำงานนอกสถานที่ / ไปทำงานต่างพื้นที่';
  }

  if (/외근/.test(c)) {
    if (/오늘/.test(c)) return p('วันนี้ออกไปทำงานนอกสถานที่');
    if (/내일/.test(c)) return p('พรุ่งนี้ออกไปทำงานนอกสถานที่');
    if (/어제/.test(c)) return p('เมื่อวานออกไปทำงานนอกสถานที่');
    if (/갔다|다녀왔|왔어요/.test(c)) return p('ออกไปทำงานนอกสถานที่มาแล้ว');
    return 'ออกไปทำงานนอกสถานที่';
  }

  if (/출근.*했/.test(c)) return p('ไปทำงานแล้ว');
  if (/출근.*하세요/.test(c)) return p('ไปทำงานนะ');
  if (/출근.*해야/.test(c)) return p('ต้องไปทำงาน');
  if (/출근.*못/.test(c)) return p('ไปทำงานไม่ได้');
  if (/퇴근.*했/.test(c)) return p('เลิกงานแล้ว');
  if (/퇴근.*하세요/.test(c)) return p('เลิกงานได้เลย');
  if (/퇴근.*해도돼/.test(c)) return q('เลิกงานได้ไหม');
  if (/일.*끝났/.test(c)) return p('งานเสร็จแล้ว');
  if (/다.*끝났/.test(c)) return p('เสร็จหมดแล้ว');
  if (/아직.*안끝/.test(c)) return p('ยังไม่เสร็จ');
  if (/야근.*있/.test(c)) return hasQ ? q('มีโอทีไหม') : p('มีโอที');
  if (/야근.*없/.test(c)) return p('ไม่มีโอที');
  if (/오늘.*야근/.test(c)) return p('วันนี้มีโอที');
  if (/내일.*야근/.test(c)) return p('พรุ่งนี้มีโอที');
  if (/잔업/.test(c)) return p('งานล่วงเวลา / โอที');
  if (/특근/.test(c)) return p('ทำงานพิเศษวันหยุด');
  if (/주말.*일/.test(c)) return p('เสาร์อาทิตย์ทำงาน');
  if (/쉬는날.*언제/.test(c)) return q('วันหยุดเมื่อไหร่');
  if (/휴무.*언제/.test(c)) return q('วันหยุดเมื่อไหร่');
  if (/월급.*언제/.test(c)) return q('เงินเดือนออกเมื่อไหร่');
  if (/월급.*들어왔/.test(c)) return p('เงินเดือนเข้าแล้ว');
  if (/월급.*안들어왔/.test(c)) return p('เงินเดือนยังไม่เข้า');
  if (/급여명세서/.test(c)) return 'สลิปเงินเดือน';
  if (/근로계약서/.test(c)) return 'สัญญาจ้างงาน';
  if (/계약.*끝|계약만료/.test(c)) return p('สัญญาหมดแล้ว');
  if (/계약.*연장/.test(c)) return p('ต่อสัญญา');
  if (/퇴사.*할/.test(c)) return p('จะลาออก');
  if (/퇴사.*하고싶/.test(c)) return p('อยากลาออก');
  if (/해고/.test(c)) return p('โดนไล่ออก');
  if (/사업장.*변경/.test(c)) return p('เปลี่ยนที่ทำงาน');
  if (/일.*못해/.test(c)) return p('ทำงานไม่ได้');
  if (/일.*할수있/.test(c)) return p('ทำงานได้');
  if (/불량/.test(c)) return p('ของเสีย / งานเสีย');
  if (/기계.*고장/.test(c)) return p('เครื่องเสีย');
  if (/작업.*확인/.test(c)) return p('ตรวจสอบงาน');
  if (/안전교육/.test(c)) return p('อบรมความปลอดภัย');
  if (/사고.*났/.test(c)) return p('เกิดอุบัติเหตุ');
  if (/다쳤/.test(c)) return p('บาดเจ็บ');
  if (/병가/.test(c)) return p('ลาป่วย');
  if (/연차/.test(c)) return p('ใช้วันลาพักร้อน');
  if (/지각/.test(c)) return p('มาสาย');
  if (/결근/.test(c)) return p('ขาดงาน');
  if (/새로운.*사람.*오/.test(c)) return p('ถ้าคนใหม่มา');
  if (/태국사람.*받/.test(c)) return q('รับคนไทยไหม');

  // ============================================================
  // 4) Housing / dorm / rent / utilities
  // ============================================================

  if (/기숙사.*들어갈수있/.test(c)) return q('เข้าหอพักได้ไหม');
  if (/오늘.*기숙사.*들어갈수있/.test(c)) return q('วันนี้เข้าหอพักได้ไหม');
  if (/기숙사.*어디/.test(c)) return q('หอพักอยู่ที่ไหน');
  if (/방.*있/.test(c)) return q('มีห้องไหม');
  if (/빈방.*있/.test(c)) return q('มีห้องว่างไหม');
  if (/원룸.*있/.test(c)) return q('มีห้องวันรูมไหม');
  if (/투룸.*있/.test(c)) return q('มีห้องทูรูมไหม');
  if (/월세.*얼마/.test(c)) return q('ค่าเช่าเท่าไหร่');
  if (/보증금.*얼마/.test(c)) return q('เงินมัดจำเท่าไหร่');
  if (/관리비.*얼마/.test(c)) return q('ค่าส่วนกลางเท่าไหร่');
  if (/관리비.*포함/.test(c)) return q('รวมค่าส่วนกลางไหม');
  if (/전기세.*얼마/.test(c)) return q('ค่าไฟเท่าไหร่');
  if (/수도세.*얼마|수도요금.*얼마/.test(c)) return q('ค่าน้ำเท่าไหร่');
  if (/가스비.*얼마|가스요금.*얼마/.test(c)) return q('ค่าแก๊สเท่าไหร่');
  if (/인터넷.*포함/.test(c)) return q('รวมอินเทอร์เน็ตไหม');
  if (/주소.*알려/.test(c)) return p('ช่วยบอกที่อยู่หน่อย');
  if (/주소.*주세요/.test(c)) return p('ขอที่อยู่หน่อย');
  if (/이사.*나가/.test(c)) return p('ย้ายออก');
  if (/이사.*들어/.test(c)) return p('ย้ายเข้า');
  if (/언제.*이사/.test(c)) return q('จะย้ายเมื่อไหร่');
  if (/보증금.*돌려/.test(c)) return p('คืนเงินมัดจำ');
  if (/보증금.*언제.*받/.test(c)) return q('จะได้เงินมัดจำคืนเมื่อไหร่');
  if (/물이.*안나와/.test(c)) return p('น้ำไม่ไหล');
  if (/전기.*나갔/.test(c)) return p('ไฟดับ');
  if (/보일러.*고장/.test(c)) return p('หม้อต้มน้ำ / ระบบทำความร้อนเสีย');
  if (/에어컨.*안돼|에어컨.*고장/.test(c)) return p('แอร์ไม่ทำงาน / แอร์เสีย');
  if (/인터넷.*안돼/.test(c)) return p('อินเทอร์เน็ตใช้ไม่ได้');

  // ============================================================
  // 5) Visa / immigration / document
  // ============================================================

  if (/비자.*연장/.test(c)) return p('ต่อวีซ่า');
  if (/비자.*변경/.test(c)) return p('เปลี่ยนวีซ่า');
  if (/비자.*신청/.test(c)) return p('ยื่นวีซ่า');
  if (/비자.*만료/.test(c)) return p('วีซ่าหมดอายุ');
  if (/외국인등록증.*잃어버/.test(c)) return p('บัตรต่างด้าวหาย');
  if (/외국인등록증.*재발급/.test(c)) return p('ทำบัตรต่างด้าวใหม่');
  if (/여권.*잃어버/.test(c)) return p('พาสปอร์ตหาย');
  if (/여권.*만료/.test(c)) return p('พาสปอร์ตหมดอายุ');
  if (/하이코리아.*예약/.test(c)) return p('จองคิว HiKorea แล้ว');
  if (/출입국.*예약/.test(c)) return p('จองคิว ตม. แล้ว');
  if (/출입국.*언제/.test(c)) return q('ไป ตม. เมื่อไหร่');
  if (/서류.*준비/.test(c)) return p('เตรียมเอกสาร');
  if (/서류.*부족/.test(c)) return p('เอกสารไม่ครบ');
  if (/서류.*제출/.test(c)) return p('ยื่นเอกสาร');
  if (/주소.*변경.*신고/.test(c)) return p('แจ้งเปลี่ยนที่อยู่');
  if (/대사관/.test(c)) return 'สถานทูต';
  if (/영사관/.test(c)) return 'สถานกงสุล';

  // ============================================================
  // 6) Bank / money / tax / insurance
  // ============================================================

  if (/계좌.*번호/.test(c)) return q('เลขบัญชีอะไร');
  if (/계좌.*개설/.test(c)) return p('เปิดบัญชี');
  if (/계좌.*해지/.test(c)) return p('ปิดบัญชี');
  if (/송금/.test(c) && /태국/.test(c)) return p('โอนเงินกลับไทย');
  if (/송금/.test(c)) return p('โอนเงิน');
  if (/입금.*했/.test(c)) return p('โอนเงินเข้าแล้ว / ฝากเงินแล้ว');
  if (/입금.*안됐/.test(c)) return p('เงินยังไม่เข้า');
  if (/잔액.*얼마/.test(c)) return q('ยอดเงินเหลือเท่าไหร่');
  if (/수수료.*있/.test(c)) return q('มีค่าธรรมเนียมไหม');
  if (/비밀번호.*잊어버/.test(c)) return p('ลืมรหัสผ่าน');
  if (/카드.*잃어버/.test(c)) return p('บัตรหาย');
  if (/체크카드.*발급/.test(c)) return p('ขอทำบัตรเช็กการ์ด / บัตรเดบิต');
  if (/거래내역서/.test(c)) return 'รายการเดินบัญชี';
  if (/잔액증명서/.test(c)) return 'ใบรับรองยอดเงิน';
  if (/국민연금.*환급/.test(c)) return p('ขอเงินกุกมินคืน');
  if (/퇴직금.*언제/.test(c)) return q('เงินแทจิกจะได้เมื่อไหร่');
  if (/세금.*환급/.test(c)) return p('คืนภาษี');
  if (/건강보험/.test(c)) return 'ประกันสุขภาพ';

  // ============================================================
  // 7) Hospital / medical / dental
  // ============================================================

  if (/아파/.test(c)) {
    if (/머리/.test(c)) return p('ปวดหัว');
    if (/배/.test(c)) return p('ปวดท้อง');
    if (/허리/.test(c)) return p('ปวดหลัง');
    if (/목/.test(c)) return p('เจ็บคอ');
    if (/이|치아/.test(c)) return p('ปวดฟัน');
    if (/무릎/.test(c)) return p('เจ็บเข่า');
    if (/어깨/.test(c)) return p('ปวดไหล่');
    if (/팔/.test(c)) return p('ปวดแขน');
    if (/다리/.test(c)) return p('ปวดขา');
    return p('เจ็บ / ปวด');
  }

  if (/열.*나/.test(c)) return p('มีไข้');
  if (/기침/.test(c)) return p('ไอ');
  if (/콧물/.test(c)) return p('มีน้ำมูก');
  if (/감기/.test(c)) return p('เป็นหวัด');
  if (/설사/.test(c)) return p('ท้องเสีย');
  if (/토했|구토/.test(c)) return p('อาเจียน');
  if (/어지러/.test(c)) return p('เวียนหัว');
  if (/숨.*못쉬/.test(c)) return p('หายใจไม่ออก');
  if (/알레르기/.test(c)) return p('ภูมิแพ้ / แพ้');
  if (/피.*나/.test(c)) return p('เลือดออก');
  if (/뼈.*부러/.test(c) || /골절/.test(c)) return p('กระดูกหัก');
  if (/디스크/.test(c)) return p('หมอนรองกระดูก / กระดูกทับเส้น');
  if (/고름/.test(c)) return p('มีหนอง / เป็นหนอง');
  if (/상처/.test(c)) return p('แผล');
  if (/염증/.test(c)) return p('อักเสบ');
  if (/입원/.test(c)) return p('นอนโรงพยาบาล');
  if (/퇴원/.test(c)) return p('ออกจากโรงพยาบาล');
  if (/수술/.test(c)) return p('ผ่าตัด');
  if (/검사/.test(c)) return p('ตรวจ');
  if (/피검사/.test(c)) return p('ตรวจเลือด');
  if (/엑스레이/.test(c)) return p('เอ็กซเรย์');
  if (/CT|씨티/.test(c)) return p('ทำ CT');
  if (/MRI|엠알아이/.test(c)) return p('ทำ MRI');
  if (/약.*먹/.test(c)) return p('กินยา');
  if (/약.*처방/.test(c)) return p('จ่ายยา / สั่งยา');
  if (/처방전/.test(c)) return 'ใบสั่งยา';
  if (/진단서/.test(c)) return 'ใบรับรองแพทย์';

  if (/사랑니/.test(c)) {
    if (/신경/.test(c) && /발치.*가능/.test(c)) return q('ฟันคุดอยู่ใกล้เส้นประสาท ถอนได้ไหม');
    if (/신경/.test(c)) return p('ฟันคุดอยู่ใกล้เส้นประสาท');
    if (/발치.*얼마/.test(c)) return q('ค่าถอนฟันคุดเท่าไหร่');
    if (/발치/.test(c)) return p('ถอนฟันคุด');
    if (/수술/.test(c)) return p('ผ่าฟันคุด');
    return 'ฟันคุด';
  }

  if (/충치/.test(c)) return p('ฟันผุ');
  if (/스케일링/.test(c)) return p('ขูดหินปูน');
  if (/임플란트/.test(c)) return p('รากฟันเทียม');
  if (/치아교정|교정/.test(c)) return p('จัดฟัน');

  // ============================================================
  // 8) Online / parcel / Coupang
  // ============================================================

  if (/쿠팡/.test(c)) return 'คู팡 / Coupang';
  if (/배송완료/.test(c) && /안보/.test(c)) return p('ขึ้นว่าส่งเสร็จแล้ว แต่ไม่เห็นพัสดุ');
  if (/택배.*안보/.test(c)) return p('ไม่เห็นพัสดุ');
  if (/문앞.*없/.test(c) || /현관앞.*없/.test(c)) return p('ไม่มีของอยู่หน้าประตู');
  if (/문앞.*놓/.test(c)) return p('วางไว้หน้าประตู');
  if (/경비실.*맡/.test(c)) return p('ฝากไว้ที่ป้อมยาม / ห้องยาม');
  if (/배송.*조회/.test(c)) return p('เช็คพัสดุ');
  if (/택배.*어디쯤/.test(c)) return q('พัสดุถึงไหนแล้ว');
  if (/택배.*아직.*도착/.test(c)) return p('พัสดุยังไม่ถึง');
  if (/택배.*분실/.test(c)) return p('พัสดุหาย');
  if (/주소.*잘못/.test(c)) return p('ที่อยู่ผิด');
  if (/다른집.*배송/.test(c)) return p('ส่งผิดบ้าน');
  if (/환불.*가능/.test(c)) return q('คืนเงินได้ไหม');
  if (/환불.*해주세요/.test(c)) return p('ขอคืนเงินหน่อย');
  if (/반품.*가능/.test(c)) return q('คืนสินค้าได้ไหม');
  if (/반품.*신청/.test(c)) return p('ขอคืนสินค้า');
  if (/교환.*가능/.test(c)) return q('เปลี่ยนสินค้าได้ไหม');
  if (/주문.*취소/.test(c)) return p('ยกเลิกออเดอร์');
  if (/결제.*했/.test(c)) return p('ชำระเงินแล้ว');
  if (/결제.*안됐/.test(c)) return p('ชำระเงินไม่สำเร็จ');
  if (/돈.*빠져나갔/.test(c) && /주문.*안보/.test(c)) return p('เงินถูกหักแล้ว แต่ไม่เห็นออเดอร์');
  if (/상품.*달라/.test(c) || /설명.*달라/.test(c) || /사진.*달라/.test(c)) return p('สินค้าไม่ตรงปก / ไม่ตรงคำอธิบาย');
  if (/파손/.test(c)) return p('สินค้าแตก / สินค้าเสียหาย');
  if (/불량/.test(c)) return p('สินค้าเสีย / สินค้าชำรุด');
  if (/오배송|잘못배송/.test(c)) return p('ส่งสินค้าผิด');
  if (/누락/.test(c)) return p('ของไม่ครบ / มีของขาด');
  if (/판매자.*연락/.test(c)) return p('ติดต่อผู้ขาย');
  if (/고객센터/.test(c)) return p('ศูนย์บริการลูกค้า');

  // ============================================================
  // 9) Mobile / SIM
  // ============================================================

  if (/유심.*있/.test(c)) return q('มีซิมการ์ดไหม');
  if (/유심.*개통/.test(c)) return p('เปิดซิม');
  if (/번호.*개통/.test(c)) return p('เปิดเบอร์');
  if (/전화번호.*뭐/.test(c)) return q('เบอร์โทรอะไร');
  if (/인증번호.*안와/.test(c)) return p('รหัสยืนยันยังไม่มา');
  if (/인증번호.*보내/.test(c)) return p('ส่งรหัสยืนยัน');
  if (/데이터.*안돼|인터넷.*안돼/.test(c)) return p('เน็ตใช้ไม่ได้');
  if (/인터넷.*느려/.test(c)) return p('เน็ตช้า');
  if (/신호.*안잡/.test(c)) return p('ไม่มีสัญญาณ');
  if (/통신사.*어디/.test(c)) return q('ใช้เครือข่ายมือถืออะไร');
  if (/요금.*얼마/.test(c)) return q('ค่าใช้จ่ายเท่าไหร่');
  if (/한달.*얼마/.test(c)) return q('เดือนละเท่าไหร่');
  if (/미납.*있/.test(c)) return q('มียอดค้างไหม');
  if (/자동이체/.test(c)) return p('หักเงินอัตโนมัติจากบัญชี');
  if (/명의.*누구/.test(c)) return q('เป็นชื่อใคร');

  // ============================================================
  // 10) Used car
  // ============================================================

  if (/중고차/.test(c)) return 'รถมือสอง';
  if (/차.*사고싶/.test(c)) return p('อยากซื้อรถ');
  if (/차.*팔고싶/.test(c)) return p('อยากขายรถ');
  if (/명의이전.*가능/.test(c)) return q('โอนชื่อรถได้ไหม');
  if (/명의이전/.test(c)) return p('โอนชื่อรถ');
  if (/주행거리.*얼마/.test(c)) return q('เลขไมล์เท่าไหร่');
  if (/사고.*이력/.test(c)) return q('มีประวัติอุบัติเหตุไหม');
  if (/무사고/.test(c)) return p('รถไม่มีอุบัติเหตุ');
  if (/침수/.test(c)) return p('รถน้ำท่วม');
  if (/성능점검기록부/.test(c)) return p('ใบตรวจสภาพรถ');
  if (/보험.*가입/.test(c)) return p('ทำประกันแล้ว');
  if (/자동차세/.test(c)) return p('ภาษีรถ');
  if (/수리비.*얼마/.test(c)) return q('ค่าซ่อมเท่าไหร่');
  if (/엔진오일.*갈/.test(c)) return p('เปลี่ยนน้ำมันเครื่อง');
  if (/할부.*가능/.test(c)) return q('ผ่อนได้ไหม');
  if (/가격.*얼마/.test(c)) return q('ราคาเท่าไหร่');
  if (/깎아.*줄수/.test(c)) return q('ลดราคาได้ไหม');

  // ============================================================
  // 11) Food / restaurant / daily
  // ============================================================

  if (/밥.*먹었/.test(c)) return hasQ ? q('กินข้าวแล้วหรือยัง') : p('กินข้าวแล้ว');
  if (/뭐.*먹을/.test(c)) return q('จะกินอะไร');
  if (/뭐.*먹고싶/.test(c)) return q('อยากกินอะไร');
  if (/배고파/.test(c)) return p('หิว');
  if (/목말라/.test(c)) return p('หิวน้ำ');
  if (/물.*주세요/.test(c)) return p('ขอน้ำหน่อย');
  if (/김치.*주세요/.test(c)) return p('ขอกิมจิหน่อย');
  if (/안맵게/.test(c)) return p('ขอไม่เผ็ด');
  if (/덜맵게/.test(c)) return p('ขอเผ็ดน้อย');
  if (/많이맵게|아주맵게/.test(c)) return p('ขอเผ็ดมาก');
  if (/포장/.test(c)) return p('ห่อกลับ');
  if (/계산/.test(c)) return p('คิดเงิน');
  if (/영업시간.*몇시/.test(c)) return q('เปิดถึงกี่โมง');
  if (/문.*닫/.test(c)) return p('ร้านปิด');
  if (/예약.*했/.test(c)) return p('จองแล้ว');
  if (/예약.*가능/.test(c)) return q('จองได้ไหม');

  // ============================================================
  // 12) School / class / study
  // ============================================================

  if (/수업.*있/.test(c)) return hasQ ? q('มีเรียนไหม') : p('มีเรียน');
  if (/수업.*없/.test(c)) return p('ไม่มีเรียน');
  if (/수업.*끝났/.test(c)) return p('เรียนเสร็จแล้ว');
  if (/수업.*시작/.test(c)) return p('เริ่มเรียน');
  if (/숙제/.test(c)) return p('การบ้าน');
  if (/과제/.test(c)) return p('งานที่ต้องส่ง / assignment');
  if (/시험/.test(c)) return p('สอบ');
  if (/출석/.test(c)) return p('เข้าเรียน / เช็กชื่อ');
  if (/결석/.test(c)) return p('ขาดเรียน');
  if (/지각/.test(c)) return p('มาสาย');
  if (/한국말.*잘해/.test(c)) return p('พูดเกาหลีเก่ง');
  if (/이해.*했/.test(c)) return p('เข้าใจแล้ว');
  if (/이해.*못/.test(c)) return p('ไม่เข้าใจ');

  // ============================================================
  // 13) Emergency / police / danger
  // ============================================================

  if (/도와주세요/.test(c)) return p('ช่วยด้วย');
  if (/살려주세요/.test(c)) return p('ช่วยชีวิตด้วย');
  if (/긴급/.test(c)) return p('ฉุกเฉิน');
  if (/구급차.*불러/.test(c)) return p('เรียกรถพยาบาลหน่อย');
  if (/경찰.*불러/.test(c)) return p('เรียกตำรวจหน่อย');
  if (/불이야|화재/.test(c)) return p('ไฟไหม้');
  if (/도둑/.test(c)) return p('ขโมย');
  if (/사기.*당했/.test(c)) return p('โดนโกง');
  if (/물건.*잃어버/.test(c)) return p('ของหาย');
  if (/지갑.*잃어버/.test(c)) return p('กระเป๋าสตางค์หาย');
  if (/휴대폰.*잃어버/.test(c)) return p('มือถือหาย');

  // ============================================================
  // 14) Beauty / clinic
  // ============================================================

  if (/쌍꺼풀|쌍수/.test(c)) return p('ทำตาสองชั้น');
  if (/코수술/.test(c)) return p('เสริมจมูก / ศัลยกรรมจมูก');
  if (/보톡스/.test(c)) return p('โบท็อกซ์');
  if (/필러/.test(c)) return p('ฟิลเลอร์');
  if (/레이저/.test(c)) return p('เลเซอร์');
  if (/리프팅/.test(c)) return p('ยกกระชับ');
  if (/지방흡입/.test(c)) return p('ดูดไขมัน');
  if (/마취/.test(c)) return p('ยาชา / ยาสลบ');
  if (/부작용/.test(c)) return p('ผลข้างเคียง');
  if (/흉터/.test(c)) return p('แผลเป็น');
  if (/붓기/.test(c)) return p('อาการบวม');

  // ============================================================
  // 15) Common question fallback
  // ============================================================

  if (/이름.*뭐/.test(c)) return q('ชื่ออะไร');
  if (/나이.*몇|몇살/.test(c)) return q('อายุเท่าไหร่');
  if (/어디서.*왔/.test(c)) return q('มาจากไหน');
  if (/태국사람/.test(c)) return p('เป็นคนไทย');
  if (/한국사람/.test(c)) return p('เป็นคนเกาหลี');
  if (/뭐해|뭐하세요/.test(c)) return q('ทำอะไรอยู่');
  if (/왜그래/.test(c)) return q('เป็นอะไร / ทำไมเป็นแบบนั้น');
  if (/왜/.test(c)) return q('ทำไม');
  if (/어떻게해/.test(c)) return q('ทำยังไงดี');
  if (/가능해/.test(c)) return q('ทำได้ไหม / เป็นไปได้ไหม');
  if (/할수있/.test(c)) return q('ทำได้ไหม');
  if (/안될까요|안돼나요|안되나요/.test(c)) return q('ไม่ได้เหรอ');
  if (/필요해/.test(c)) return p('จำเป็น / ต้องใช้');
  if (/필요없/.test(c)) return p('ไม่จำเป็น / ไม่ต้องใช้');
  if (/좋아요/.test(c)) return hasQ ? q('ดีไหม') : p('ดี');
  if (/싫어요/.test(c)) return p('ไม่ชอบ');
  if (/사랑해/.test(c)) return p('รักคุณ');
  if (/보고싶/.test(c)) return p('คิดถึง');
  if (/잘자/.test(c)) return p('ฝันดี');
  if (/내일봐/.test(c)) return p('พรุ่งนี้เจอกัน');
